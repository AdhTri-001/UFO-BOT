...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    def skip_to(self, time:str):
        ...
        if len(timelist) == 2:
            timetoreturn= int(time[1]) * 60 + int(time[0])
            return timetoreturn
        if len(timelist) == 3:
            timetoreturn= int(time[2]) * 3600 + int(time[1]) * 60 + int(time[0])
            return timetoreturn
    ...
    async def has_modrole(self, ctx):
        ...
        try:
            ...
            return False
    async def can_mod_channel(self, ctx):
        try:
            channel: discord.VoiceChannel= ctx.me.voice_state.channel
        except:
            return False
    ...
    async def on_player_stop(self, node: wavelink.Node, payload):
        payload.player.now_playing= None
    ...
    async def vote(self, ctx):
        ...
        if not ctx.author.id in player.listners:
            await ctx.send('You are not joined to the voice channel. Do the need full first.')
    ...
    async def force(self, ctx):
        ...
        if await self.has_requested(ctx) or await self.has_modrole(ctx) or await self.can_mod_channel(ctx):
            pass
        else:
    ...
    async def seek(self, ctx, timeToSeek):
        ...
        if pos> player.now_playing.length//1000:
            pos= player.now_playing.length//1
        await player.seek(position= pos)
        embed= discord.Embed(
            title= "Seek!",
